...
import './IMDXFactory.sol';
import './IMDXRouter02.sol';
import './IMDXPair.sol';
import './IWMasterChef.sol';
import './IACMDMining.sol';


contract MdxswapSpellV2 is WhitelistSpell {

  using SafeMath for uint;
  using ArchimedesMath for uint;

  IMDXFactory public immutable factory;
  IMDXRouter02 public immutable router;

  mapping(address => mapping(address => address)) public pairs;

  IWMasterChef public immutable wmasterchef;

  address public immutable mdx;
  address public immutable mining;

  constructor(
    IBank _bank,
    address _werc20,
    IMDXRouter02 _router,
    address _wmasterchef,
    address _mining
  ) public WhitelistSpell(_bank, _werc20, _router.WHT()) {
    router = _router;
    factory = IMDXFactory(_router.factory());
    wmasterchef = IWMasterChef(_wmasterchef);
    IWMasterChef(_wmasterchef).setApprovalForAll(address(_bank), true);
    mdx = address(IWMasterChef(_wmasterchef).mdx());
    mining = _mining;
  }
  ...
  function addLiquidityInternal(
    ...
    bool isReversed;
    {
      ...
      uint resA;
      uint resB;
      if (IMDXPair(lp).token0() == tokenA) {
        (resA, resB, ) = IMDXPair(lp).getReserves();
      } else {
        (resB, resA, ) = IMDXPair(lp).getReserves();
      }
  ...
  function addLiquidityWERC20(
    ...
    (address user, , , ) = bank.getPositionInfo(positionId);

    IACMDMining(mining).rewardPosition(positionId, tokenA, tokenB);
  ...
  function addLiquidityWMasterChef(
    ...
    uint positionId = bank.POSITION_ID();
    (address user, , uint collId, uint collSize) = bank.getPositionInfo(positionId);
  ...
  function removeLiquidityWMasterChef(
    ...
    address lp = getPair(tokenA, tokenB);
    uint positionId = bank.POSITION_ID();
    (address user, address collToken, uint collId, ) = bank.getPositionInfo(positionId);
