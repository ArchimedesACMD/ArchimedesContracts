...
contract ProxyOracle is IOracle, Governable {
  ...
  function convertForLiquidation(
    ...
    uint pxIn = source.getUSDPx(tokenIn);     
    uint pxOut = source.getUSDPx(tokenOutUnderlying);  //
  ...
  /// @dev Return the value of the given input as USD for collateral purpose.


  function asUSDCollateral(
    address token,
    uint id,
    uint amount, 
    address owner
    ...
    uint ethValue = source.getUSDPx(tokenUnderlying).mul(amountUnderlying).div(1e18); 
    return ethValue.mul(oracle.collateralFactor).div(10000); 
  }

  /// @dev Return the value of the given input as USD for borrow purpose.

  function asUSDBorrow(
    address token,
    uint amount,
    address owner
  ) external view override returns (uint) {
    ...
    if (token == 0x0298c2b32eaE4da002a15f36fdf7615BEa3DA047) {   //HUSD
        uint ethValue = source.getUSDPx(token).mul(amount).div(1e8); 
        return ethValue.mul(oracle.borrowFactor).div(10000);  
    } else {
        uint ethValue = source.getUSDPx(token).mul(amount).div(1e18); 
        return ethValue.mul(oracle.borrowFactor).div(10000); 
    }
  }
}
