...
contract ArchimedesBank is Initializable, Governable, ERC1155NaiveReceiver, IBank {
  ...
  struct Bank {
    ...
    uint reserve; // The reserve portion allocated to Archimedes protocol.
  ...
  /// @param _feeBps The fee collected to Archimedes bank.
  function initialize(IOracle _oracle, uint _feeBps) external initializer {
    ...
    POSITION_ID = 0;
    SPELL = _NO_ADDRESS;
    caster = address(new ArchimedesCaster());
    oracle = _oracle;
  ...
  function setWhitelistSpells(address[] calldata spells, bool[] calldata statuses) external onlyGov
  {
    require(spells.length < 5, "too many spells to whitelist");
  ...
  /// @param statuses list of statuses to change to
  function setWhitelistTokens(address[] calldata tokens, bool[] calldata statuses) external onlyGov
  {
    require(tokens.length < 10, "too many tokens to whitelist");
  ...
  function setWhitelistUsers(address[] calldata users, bool[] calldata statuses) external onlyGov {
    require(users.length < 5, "too many users to whitelist");
  ...
  function accrueAll(address[] memory tokens) external {
    require(tokens.length <= allBanks.length);
    for (uint idx = 0; idx < tokens.length; idx++) {
      accrue(tokens[idx]);
    }
  }
  ...
  function getPositionDebtShareOf(uint positionId, address token) external override view returns (uint) {
  ...
  /// @dev Return the total collateral value of the given position in USD.
  /// @param positionId The position ID to query for the collateral value.
  function getCollateralUSDValue(uint positionId) public view returns (uint) {
    Position storage pos = positions[positionId];
    ...
    if (size == 0) {
      ...
      return oracle.asUSDCollateral(pos.collToken, pos.collId, size, pos.owner);
    }
  }

  /// @dev Return the total borrow value of the given position in USD.
  /// @param positionId The position ID to query for the borrow value.
  function getBorrowUSDValue(uint positionId) public view override returns (uint) {
    uint value = 0;
    Position storage pos = positions[positionId];
    ...
    while (bitMap > 0) {
      if ((bitMap & 1) != 0) {
        ...
        value = value.add(oracle.asUSDBorrow(token, debt, owner));
      }
      idx++;
      bitMap >>= 1;
    }
  ...
  /// @dev Set the fee bps value that Archimedes bank charges.
  ...
  function liquidate(
    ...
    uint collateralValue = getCollateralUSDValue(positionId);
    uint borrowValue = getBorrowUSDValue(positionId);
  ...
  /// @dev Execute the action via ArchimedesCaster, calling its function with the supplied data.
  /// @param positionId The position ID to execute the action, or zero for new position.
  /// @param spell The target spell to invoke the execution via ArchimedesCaster.
  ...
  function execute(
    ...
    SPELL = spell;
    ArchimedesCaster(caster).cast{value: msg.value}(spell, data);
    uint collateralValue = getCollateralUSDValue(positionId);
    uint borrowValue = getBorrowUSDValue(positionId);
